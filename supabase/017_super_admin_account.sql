-- ============================================================================
-- 017 — SUPER ADMIN ACCOUNT (STRICT READ-ONLY COMPLIANCE & VETTING)
-- ============================================================================
...
-- 2. Helper function to check if the current user has the auditor/super-admin read-only role
...
-- 3. Create or update the Super Admin user account
do $$
declare
  v_superadmin_email text := 'superadmin@uniguard.co.uk';
  v_superadmin_pass  text := 'SuperAdminSecure2026!'; -- Set your secure password here
  v_uid uuid;
begin
  select id into v_uid from auth.users where email = v_superadmin_email;
  ...
  if v_uid is null then
    ...
    values
      ('00000000-0000-0000-0000-000000000000', v_uid, 'authenticated', 'authenticated',
       v_superadmin_email, crypt(v_superadmin_pass, gen_salt('bf')), now(),
       jsonb_build_object('provider', 'email', 'providers', jsonb_build_array('email')),
       jsonb_build_object('full_name', 'Super Admin'), now(), now());

    insert into auth.identities
      (id, provider_id, user_id, identity_data, provider, last_sign_in_at, created_at, updated_at)
    values
      (gen_random_uuid(), v_uid::text, v_uid,
       jsonb_build_object('sub', v_uid::text, 'email', v_superadmin_email),
       'email', now(), now(), now());
  else
    update auth.users
       set encrypted_password = crypt(v_superadmin_pass, gen_salt('bf'))
     where id = v_uid;
    delete from auth.sessions where user_id = v_uid;
  end if;

  -- Ensure profile row with is_auditor = true and is_admin = false (strict read-only audit mode)
  insert into public.profiles (id, email, full_name, is_admin, is_auditor)
  values (v_uid, v_superadmin_email, 'Super Admin', false, true)
  on conflict (id) do update
    set email = v_superadmin_email, is_admin = false, is_auditor = true;
end $$;

-- 4. RLS Policies: Grant Super Admin READ-ONLY (SELECT) access across tables
